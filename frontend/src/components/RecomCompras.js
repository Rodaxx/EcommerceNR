...
const RecomendacionesCompras = () => {
  const [productos, setProductos] = useState([]);

  const getRecomendacionesCompras = async () => {
    try {
      const response = await fetch('http://localhost:2000/recomendations/purchase');
  ...
  return (
    <div className="mt-3 text-lg bg-gray flex justify-center">
      <div className="p-2 bg-gray-100 w-4/5 rounded-lg">
        <h2 className="px-4 py-2">Algunas recomendaciones en base a tus compras...</h2>
...
export default RecomendacionesCompras;
